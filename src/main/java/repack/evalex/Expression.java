 ...
 *
 * <ul>
 *   <li>Uses BigDecimal for calculation and result
 *   <li>Single class implementation, very compact
 *   <li>No dependencies to external libraries
 *   <li>Precision and rounding mode can be set
 *   <li>Supports variables
 *   <li>Standard boolean and mathematical operators
 *   <li>Standard basic mathematical and boolean functions
 *   <li>Custom functions and operators can be added at runtime
 * </ul>
 *
 * <br>
 *
 ...
 * </table>
 *
 * *Boolean operators result always in a BigDecimal value of 1 or 0 (zero). Any non-zero value is
 * treated as a _true_ value. Boolean _not_ is implemented by a function. <br>
 *
 ...
 *
 * Custom operators can be added easily, simply create an instance of `Expression.Operator` and add
 * it to the expression. Parameters are the operator string, its precedence and if it is left
 * associative. The operators `eval()` method will be called with the BigDecimal values of the
 * operands. All existing operators can also be overridden. <br>
 * For example, add an operator `x >> n`, that moves the decimal point of _x_ _n_ digits to the
 * right:
 *
 * <pre>
 ...
 *
 * Adding custom functions is as easy as adding custom operators. Create an instance of
 * `Expression.Function`and add it to the expression. Parameters are the function name and the count
 * of required parameters. The functions `eval()` method will be called with a list of the
 * BigDecimal parameters. All existing functions can also be overridden. <br>
 * For example, add a function `average(a,b,c)`, that will calculate the average value of a, b and
 * c: <br>
 *
 * <pre>
 ...
 *       Overflow](http://stackoverflow.com/questions/3579779/how-to-do-a-fractional-power-on-bigdecimal-in-java)
 *       Thanks to Gene Marin
 *   <li>The SQRT() function implementation was taken from the book [The Java Programmers Guide To
 *       numerical
 *       Computing](http://www.amazon.de/Java-Number-Cruncher-Programmers-Numerical/dp/0130460419)
 *       (Ronald Mak, 2002)
 * </ul>
 *
...
public class Expression {
    ...
    public abstract class Function {
         ...
         *
         * @param parameters Parameters will be passed by the expression evaluator as a {@link List}
         *     of {@link BigDecimal} values.
    ...
    public abstract class Operator {
         ...
         * @param leftAssoc <code>true</code> if the operator is left associative, else <code>false
         *     </code>.
         */
    ...
    private class Tokenizer implements Iterator<String> {
        ...
        public String next() {
            ...
            if (Character.isDigit(ch)) {
                while ((Character.isDigit(ch) || ch == Expression.this.decimalSeparator)
                        && this.pos < this.input.length()) {
                    token.append(this.input.charAt(this.pos++));
                    ch = this.pos == this.input.length() ? 0 : this.input.charAt(this.pos);
                }
            } else if (ch == Expression.this.minusSign
                    && Character.isDigit(this.peekNextChar())
                    && ("(".equals(this.previousToken)
                            || ",".equals(this.previousToken)
                            || this.previousToken == null
                            || Expression.this.operators.containsKey(this.previousToken))) {
                token.append(Expression.this.minusSign);
                this.pos++;
                ...
                while (!Character.isLetter(ch)
                    ...
                    this.pos++;
                    ch = this.pos == this.input.length() ? 0 : this.input.charAt(this.pos);
                    if (ch == Expression.this.minusSign) {
                        break;
                    }
                }
     ...
     *
     * @param expression The expression. E.g. <code>"2.4*sin(3)/(2-4)"</code> or <code>
     *     "sin(y)>0 & max(z, 3)>3"</code>
     */
    public Expression(String expression) {
                        ...
                        if (signOf2 == -1) {
                            result =
                                    BigDecimal.ONE.divide(
                                            result,
                                            Expression.this.mc.getPrecision(),
        ...
        this.addOperator(
                new Operator("==", 7, false) {
                    @Override
                    public BigDecimal eval(BigDecimal v1, BigDecimal v2) {
                        return Expression.this.operators.get("=").eval(v1, v2);
                    }
                });
        ...
        this.addFunction(
                new Function("SQRT", 1) {
                    ...
                    public BigDecimal eval(List<BigDecimal> parameters) {
                        ...
                        BigInteger n =
                                x.movePointRight(Expression.this.mc.getPrecision() << 1)
                                        .toBigInteger();
    ...
    private boolean isNumber(String st) {
        if (st.charAt(0) == this.minusSign && st.length() == 1) return false;
        for (char ch : st.toCharArray()) {
            if (!Character.isDigit(ch) && ch != this.minusSign && ch != this.decimalSeparator)
                return false;
        }
        return true;
    }
    ...
    /**
     * Implementation of the <i>Shunting Yard</i> algorithm to transform an infix expression to a
     * RPN expression.
     *
    ...
    private List<String> shuntingYard(String expression) {
        ...
        while (tokenizer.hasNext()) {
            ...
            if (this.isNumber(token)) {
                ...
                if (stack.isEmpty()) {
                    throw new ExpressionException(
                            "Parse error for function '" + lastFunction + "'");
                }
                ...
                while (this.operators.containsKey(token2)
                        && (o1.isLeftAssoc()
                                        && o1.getPrecedence()
                                                <= this.operators.get(token2).getPrecedence()
                                || o1.getPrecedence()
                                        < this.operators.get(token2).getPrecedence())) {
                    outputQueue.add(stack.pop());
                ...
                stack.pop();
                if (!stack.isEmpty() && this.functions.containsKey(stack.peek().toUpperCase())) {
                    outputQueue.add(stack.pop());
                }
            }
            previousToken = token;
    ...
    public Expression setVariable(String variable, String value) {
        if (this.isNumber(value)) this.variables.put(variable, new BigDecimal(value));
        else {
            this.expression =
                    this.expression.replaceAll("\\b" + variable + "\\b", "(" + value + ")");
            this.rpn = null;
        }
        return this;
    }
     ...
     * per expression instance. If no cached instance exists, a new one will be created and put to
     * the cache.
     *
